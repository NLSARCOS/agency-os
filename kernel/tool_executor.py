...
class ToolExecutor:
    ...
    def __init__(self) -> None:
        ...
        self._agent_permissions: dict[str, set[ToolPermission]] = {
            "dev": {ToolPermission.READ, ToolPermission.WRITE, ToolPermission.EXECUTE,
                    ToolPermission.GIT, ToolPermission.NETWORK},
            "leadops": {ToolPermission.READ, ToolPermission.WRITE,
                        ToolPermission.NETWORK},
            "marketing": {ToolPermission.READ, ToolPermission.WRITE,
                          ToolPermission.NETWORK},
            "sales": {ToolPermission.READ, ToolPermission.WRITE,
                      ToolPermission.NETWORK},
            "abm": {ToolPermission.READ, ToolPermission.WRITE,
                    ToolPermission.NETWORK},
            "analytics": {ToolPermission.READ, ToolPermission.NETWORK},
            "creative": {ToolPermission.READ, ToolPermission.WRITE,
                         ToolPermission.NETWORK},
        }
    ...
    def check_permission(
        ...
        agent_perms = self._agent_permissions.get(
            agent_id, {ToolPermission.READ}
        )
    ...
    def execute(
        ...
        logger.info(
            ...
            tool_name, "OK" if result.success else "FAIL",
            result.duration_ms, agent_id,
        )
        return result
    ...
    def _tool_read_file(self, params: dict, **kw: Any) -> ToolResult:
        ...
        return ToolResult(
            tool="read_file", success=True, output=content,
    ...
    def _tool_http_request(
        self, params: dict, timeout: float = 30.0
    ) -> ToolResult:
        ...
        try:
            ...
            return ToolResult(
                tool="http_request", success=resp.is_success,
                output=output,
                metadata={
    ...
    def _tool_db_query(self, params: dict, **kw: Any) -> ToolResult:
        ...
        try:
            ...
            return ToolResult(
                tool="db_query", success=False, error=str(e),
            )
    ...
    def _tool_db_execute(self, params: dict, **kw: Any) -> ToolResult:
        ...
        try:
            ...
            conn.close()
            return ToolResult(
                tool="db_execute", success=True,
                output=json.dumps(result),
                metadata=result,
            )
        except Exception as e:
            return ToolResult(
                tool="db_execute", success=False, error=str(e),
            )
    ...
    def _tool_git_commit(self, params: dict, **kw: Any) -> ToolResult:
        ...
        if not stage.success:
            return stage
        return self._tool_shell(
            {"command": f'git commit -m "{message}"', "cwd": cwd}
        )
    ...
    def _tool_git_push(self, params: dict, **kw: Any) -> ToolResult:
        ...
        cwd = params.get("cwd", str(self.cfg.root))
        return self._tool_shell(
            {"command": f"git push {remote} {branch}", "cwd": cwd}
        )
    ...
    def _tool_search_web(self, params: dict, **kw: Any) -> ToolResult:
        ...
        try:
            ...
            return ToolResult(
                tool="search_web", success=False, error=str(e),
            )
    ...
    def _tool_scrape_url(self, params: dict, **kw: Any) -> ToolResult:
        ...
        try:
            ...
            return ToolResult(
                tool="scrape_url", success=True, output=text,
                metadata={"url": url, "length": len(text)},
            )
        except Exception as e:
            return ToolResult(
                tool="scrape_url", success=False, error=str(e),
            )
    ...
    def get_history(self, tool: str | None = None, limit: int = 50) -> list[dict]:
        ...
        return [
            {
                "id": r.id, "tool": r.tool, "success": r.success,
