...
@pytest.mark.asyncio
@mock.patch("kernel.heartbeat.AgencyHeartbeat._run_hustle_cycle")
@mock.patch("kernel.heartbeat.AgencyHeartbeat._run_evolution_cycle")
@mock.patch("kernel.heartbeat.AgencyHeartbeat._run_learning_cycle")
@mock.patch("kernel.heartbeat.AgencyHeartbeat._run_autonomy_cycle")
@mock.patch("kernel.heartbeat.AgencyHeartbeat._run_mission_cycle")
@mock.patch("kernel.heartbeat.AgencyHeartbeat._run_scheduled_tasks")
async def test_heartbeat_tick_logic(
    m_sched, m_mission, m_auto, m_learn, m_evolve, m_hustle
):
    ...
    hb.last_evolution = 0 # Force evolution

    # Just run a single tick to ensure no syntax errors and state updates
    await hb._tick()
    assert hb.last_hustle > 0
    assert hb.last_evolution > 0
    m_hustle.assert_called_once()
    m_evolve.assert_called_once()
...
def test_skill_evaluator_tracking():
    ...
    for _ in range(5):
        evaluator._on_phase_complete(
            type(
                "MockEvent", (), {"payload": {"studio": "dev", "status": "completed"}}
            )()
        )
    ...
    for _ in range(3):
        evaluator._on_phase_complete(
            type(
                "MockEvent", (), {"payload": {"studio": "sales", "status": "failed"}}
            )()
        )
